...
- фиксировать моментальную отправку денег второй стороне без подтверждения
...
- валюта по умолчанию выбирается через `/setcurrency` и потом используется в новых операциях
- `Добавить трату` создает `pending` запрос, который вторая сторона подтверждает или отклоняет
- `Отправить` сразу создает подтвержденную операцию и отправляет второй стороне уведомление
- если в `Добавить трату` выражение суммы дало отрицательный результат, операция автоматически считается отправкой денег
- сумму можно вводить выражением, например `35/2 + 12 - 5`
- для трат можно выбрать быструю категорию с клавиатуры или ввести описание вручную
- через `/trust` можно включить автоподтверждение трат для текущей пары
